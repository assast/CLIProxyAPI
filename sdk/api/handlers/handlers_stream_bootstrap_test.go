...
func TestExecuteStreamWithAuthManager_SoftPinnedAuthFallsBackToNextUpstream(t *testing.T) {
	executor := &authAwareStreamExecutor{}
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(executor)
	...
	ctx := WithPinnedAuthSoftFallback(WithPinnedAuthID(context.Background(), "auth1"))
	dataChan, _, errChan := handler.ExecuteStreamWithAuthManager(ctx, "openai", "test-model", []byte(`{"model":"test-model"}`), "")
	if dataChan == nil || errChan == nil {
		t.Fatalf("expected non-nil channels")
	}
	...
	authIDs := executor.AuthIDs()
	if len(authIDs) != 2 {
		t.Fatalf("expected two upstream attempts, got %v", authIDs)
	}
	if authIDs[0] != "auth1" || authIDs[1] != "auth2" {
		t.Fatalf("expected fallback sequence [auth1 auth2], got %v", authIDs)
	}
}
